...
class MaskJsonUtils():
    ...
    def add_mask(self, image_path, mask_path, color_categories):
        # Takes an image path, its corresponding mask path, and its color categories,and adds it to the appropriate dictionaries
    ...
    def get_super_categories(self):
        # Returns a dictionary of lists of categories keyed on super_category
...
class ImageComposition():
    ...
    def _generate_images(self):
        ...
        for i in tqdm(range(self.count)): #loop through the number of images we defined earlier
    ...
    def _compose_images(self, foregrounds, background_path):
        ...
        for fg in foregrounds:
            ...
            new_alpha_mask = Image.new('L', composite.size, color = 0) # L means gray scale
            new_alpha_mask.paste(alpha_mask, paste_position)
            composite = Image.composite(new_fg_image, composite, new_alpha_mask) # the alpha layer defines what's visiable on top
    ...
    def _transform_foreground(self, fg, fg_path):
        ...
        # more transformation can be added here
